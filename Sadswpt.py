...
async def on_message(message):
    ...
    if message.content.lower().startswith("s!sorteio"):
        ...
        mensagem = discord.Embed(
            title='🎉 SORTEIOS 🎉',
            color=preto,
            description='Tem um sorteio relâmpago acontecendo. **Para participar basta reagir** 🎉\n'
                        '\nSolicitem o cargo <@&465968428457656351> no <#465744338786189332> para está sempre por dentro de todos os sorteios.\n'
                        '\nVocê foi marcado por: **{}**. <:grrr:470022048257671190>\n\n**Boa sorte!**'.format(msg.name),
        )
        await client.send_message(canal, embed=mensagem)
